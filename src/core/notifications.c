...
#ifdef __APPLE__
    char command[1024];
    // Безопасное форматирование с проверкой размера
    if (snprintf(command, sizeof(command),
             "osascript -e 'display notification \"%s\" with title \"SysMon\" subtitle \"%s\"'",
             message, title) >= (int)sizeof(command)) {
        // Если переполнение, используем усеченный вариант
        snprintf(command, sizeof(command), "osascript -e 'display notification \"%s\" with title \"SysMon\"'", message);
    }
