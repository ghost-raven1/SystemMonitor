...
const char *get_network_info() {
    ...
    // fallback using nettop if zero and tool is available
    if (rx_bps == 0.0f && tx_bps == 0.0f) {
        FILE *fp = popen("nettop -L 1 -x -P -J bytes_in,bytes_out 2>/dev/null | awk -F',' 'NR>1{rin+=$2;rout+=$3} END{print rin, rout}'", "r");
        if (fp) {
            double rin=0, rout=0; if (fscanf(fp, "%lf %lf", &rin, &rout) == 2) { rx_bps = (float)rin; tx_bps = (float)rout; }
            pclose(fp);
        }
    }
